...
namespace BEPUutilities
{
    ...
    public class SpinLock
    {
        ...
        internal void WaitBriefly(ref int attempt)
        {
            ...
            {
                ThreadSpinWait(Math.Min(3 << attempt, MaximumSpinWait));
                //Thread.SpinWait(Math.Min(3 << attempt, MaximumSpinWait));
            }
        }

        private void ThreadSpinWait(int attempt)
        {
            argStore[0] = attempt;
            spinWaitMethod.Invoke(null, argStore);
        }

        private object[] argStore = new object[1];
        private MethodInfo spinWaitMethod = typeof(Thread).GetTypeInfo().GetDeclaredMethod("SpinWait");
    }
}