    ...
    public class MobileMeshCollidable : EntityCollidable
    {
        ...
        public override bool RayCast(Ray ray, float maximumLength, out RayHit rayHit)
        {
            ...
            if (Shape.solidity == MobileMeshSolidity.Solid)
            {
                ...
                {
                    //It was inside!
                    rayHit = new RayHit() { Location = ray.Position, Normal = System.Numerics.Vector3.Zero, T = 0 };
                    return true;
        ...
        public override bool ConvexCast(ConvexShape castShape, ref RigidTransform startingTransform, ref System.Numerics.Vector3 sweep, out RayHit hit)
        {
            if (Shape.solidity == MobileMeshSolidity.Solid)
            {
                ...
                if (Shape.IsLocalRayOriginInMesh(ref ray, out hit))
                {
                    ...
                    hit = new RayHit() { Location = startingTransform.Position, Normal = new System.Numerics.Vector3(), T = 0 };
                    return true;
                }
            }
            hit = new RayHit();
